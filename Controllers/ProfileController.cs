...
namespace DotNetApi.Controllers
{
    ...
    public class ProfileController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateProfilePicture(
            [FromForm] ProfilePictureRequest model
        )
        {
            ...
            if (result)
            {
                if (!string.IsNullOrWhiteSpace(oldPath) && !oldPath.Contains("default-avatar.jpg"))
                {
                    var fullOldPath = Path.Combine(
                        Directory.GetCurrentDirectory(),
                        "wwwroot",
                        oldPath.TrimStart('/').Replace("/", Path.DirectorySeparatorChar.ToString())
                    );
            ...
            else
            {
                ...
                return BadRequest(
                    new { status = false, message = "Failed to update profile picture." }
                );
            }
        }
        ...
        public IActionResult UpdateProfile(int userid, [FromBody] ProfileUpdateRequest model)
        {
            ...
            {
                return Unauthorized(
                    new { status = false, message = "Invalid user ID. Authentication required." }
                );
            }
            ...
            {
                return BadRequest(
                    new { status = false, message = "Invalid request. No data provided." }
                );
            }
            ...
            string newEmail = string.IsNullOrWhiteSpace(model.Email)
                ? currentUser.Email
                : model.Email;
            ...
            return Ok(
                new { status = true, message = "Your profile has been updated successfully." }
            );
        }
